...
A number of functions within the ROM have been decompiled to a state where they are functionally equivalent, but are not a byte-perfect match. Progress is approximately 2% ahead of the fully matching build. In order to build/test the non-matching, add `NON_MATCHING=1` argument to the `make` commands.
