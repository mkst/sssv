...
extern ALMicroTime      __CSPVoiceHandler(void *node);
extern void             __CSPHandleNextSeqEvent(ALCSPlayer *seqp);
extern void             __CSPHandleMIDIMsg(ALCSPlayer *seqp, ALEvent *event);
extern void             __CSPHandleMetaMsg(ALCSPlayer *seqp, ALEvent *event);
...
#ifdef SSSV
                if (((chan != 12) || ((s8)D_80286458 == 0)) &&
                    (((s8)D_80286458 != 0) || (chan < 9) || (chan > 11)))
                {
#endif
...
void __setUsptFromTempo(ALCSPlayer *seqp, f32 tempo) {
    ...
    if (D_801546E0 == 0) {
        ...
        tmp = D_801546E0 / (f32)2048;
    }
    if (seqp->target) {
        seqp->uspt = (s32)((f32)tempo * seqp->target->qnpt) / tmp;
        D_80286568 = (s32)((f32)tempo * seqp->target->qnpt);
    } else {
        seqp->uspt = 488.0f / tmp;
        D_80286568 = 488.0f;
    }
#else
    if (seqp->target)
...
void func_80135518(ALCSPlayer *seqp) {
    ...
    if (seqp->state == AL_PLAYING) {
        if (D_80286564 != D_801546E0) {
            D_80286564 = D_801546E0;
            tmp = D_801546E0 / (f32)2048;
            if (tmp == 0.0f) {
                tmp = 1.0f;
            }
            seqp->uspt = (D_80286568 / tmp);
        }
    }
}
