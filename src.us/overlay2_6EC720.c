...
void interpolate_bone_from_previous_pose(u16 src, u16 dst, s16 yawIndex, s16 pitchIndex, u16 boneLength, u16 interpStep) {
    ...
    targetZ = (SINB(yawIndex) * boneLength) / 256;
    targetY = (COSB(yawIndex) * boneLength) / 256;

    targetX = (SINB(pitchIndex) * targetZ) / 256;
    targetZ = (COSB(pitchIndex) * targetZ) / 256;
...
void func_802DB494_6ECB44(u16 src, u16 dst, s16 yawIndex, s16 pitchIndex, u16 boneLength) {
    ...
    tmp1 = (SINB(yawIndex) * boneLength) / 256;
    tmp3 = (COSB(yawIndex) * boneLength) / 256;
    tmp2 = (SINB(pitchIndex) * tmp1) / 256;
    tmp1 = (COSB(pitchIndex) * tmp1) / 256;
